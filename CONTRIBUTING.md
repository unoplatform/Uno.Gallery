...
- **`SampleManifest.g.cs`** — `internal static class SampleManifest` exposing
  ...
  JSON catalog (schema version 2) for the current compilation target. The
  original schema-v1 file remains committed for existing consumers.
...
### Stable sample-detail contract

New samples that explicitly set `Status = SampleStatus.Stable` must also set
`ContractVersion = 1`. The generator reports `UGG0011` as a build error when a
contract is incomplete. Contract v1 requires:

- a description, official documentation link, tags, owner, and an ISO
  `ReviewedOn` date (`YYYY-MM-DD`);
- nonzero typed `SupportedDesigns` and `SupportedRenderers` flags;
- requirements, accessibility notes, reset behavior, and meaningful variants.

Use `SampleContractDefaults.NoExternalRequirements` or
`SampleContractDefaults.ReloadToReset` only when that statement is literally
true. Accessibility notes and variants must describe the page rather than use a
generic default. Optional known limitations, issue links, and API links appear
only when authored. Legacy pages whose Stable status is implicit remain visible
in the manifest backlog and must not be marked reviewed without an actual
contract review.

Catalog validation publishes `sample-contract-report.json` next to each
target-specific manifest. It fails on incomplete contract-v1 entries, explicit
Stable entries outside contract v1, or regressions in the target baseline.
See [ADR 0007](docs/decisions/0007-enforceable-sample-detail-contract.md).
